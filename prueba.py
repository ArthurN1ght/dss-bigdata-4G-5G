...
# --- 1. CONFIGURACIÓN SIDEBAR (Subida opcional para el usuario) ---
with st.sidebar:
    st.header("📂 Carga de Archivos")
    file_trafico = st.file_uploader("Cargar Data11.csv (Opcional)", type=["csv"])
    file_geo = st.file_uploader("Cargar 2025 Sites 1900.kml (Opcional)", type=["kml"])
    st.divider()
...
# --- LÓGICA INTELIGENTE DE MODALIDAD DEMO (DATOS POR DEFECTO) ---
# Si el usuario NO sube un archivo, le indicamos a Python que lea el archivo local que estará en tu GitHub.
# Si el usuario SÍ sube un archivo, usará el que el usuario arrastró.

if file_trafico is None:
    path_trafico = "Data11.csv"  # Buscará este nombre exacto en la carpeta de tu GitHub
    st.sidebar.info("💡 Modo Demo: Usando datos de tráfico por defecto.")
else:
    path_trafico = file_trafico

if file_geo is None:
    path_geo = "2025 Sites 1900.kml"  # Buscará este nombre exacto en la carpeta de tu GitHub
    st.sidebar.info("💡 Modo Demo: Usando mapa KML por defecto.")
else:
    path_geo = file_geo


# --- 2. EXTRACCIÓN GEOGRÁFICA (KML) ---
def extraer_geo_kml(file_source):
    try:
        # Adaptación para leer si es un objeto subido o una ruta de archivo local
        if isinstance(file_source, str):
            with open(file_source, 'rb') as f:
                content = f.read()
        else:
            content = file_source.read()
            file_source.seek(0) # Resetear puntero por si acaso
...
# Evaluamos que existan datos cargados (ya sean los del usuario o los de por defecto)
if path_trafico and path_geo:
    geo_map = extraer_geo_kml(path_geo)
    
    # Lectura y limpieza del dataset de tráfico (Acepta string de ruta o archivo cargado)
    try:
        pd_raw = pd.read_csv(path_trafico, sep=';', on_bad_lines='skip')
    except Exception as e:
    ...
    if col_site in pd_raw.columns and col_traf in pd_raw.columns:
        ...
        if sitios_validos:
            ...
            st.error("❌ Error de Vínculo: Los nombres en el CSV y el KML son incompatibles o el KML está vacío.")
    else:
        st.error("❌ Columnas no encontradas. Asegúrate de que el CSV tenga 'ManagedElement Name' y 'LTE PS Traffic'.")
